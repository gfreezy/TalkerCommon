...
public actor TextLayoutManager: NSObject, NSTextContentStorageDelegate, NSTextLayoutManagerDelegate
{
    ...
    public func setText(attributedString: consuming NSAttributedString, layoutSize: CGSize) {
    ...
    public func getLineRectsByCharacterIndex(start: Int, end: Int? = nil) -> [CGRect] {
        ...
        let endPosition =
            if let end {
                textLayoutManager.location(startPosition, offsetBy: end)
            } else {
...
private struct TextLayoutManagerPreview: View {
    @State var width: CGFloat = 20
    @State var height: CGFloat = 20
    @State var rects: [TextLineSegnemt] = []
    @State var textLayoutManager: TextLayoutManager = TextLayoutManager()
